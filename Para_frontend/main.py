...
default_settings = {
    ...
    'tasks': [
        ...
        {'name': 'Burn', 'command': '/usr/bin/burnCortexA8'},
    ...
    'task_kill_image': 'img/kill_button.png',
...
class ManyMan(App):
    ...
    def config_kivy(self):
        ...
        if Config.get('kivy', 'keyboard_mode') != 'systemandmulti' or \
    ...
    def init_core_grid(self):
        ...
        ARM_core_grid = GridLayout(cols=2, rows=1, spacing=20, padding=10, 
                                    size_hint_y=0.25)
        for i in orientation[0]:
            c = Core(i, self)
            self.cores[i] = c
            ARM_core_grid.add_widget(c)
            Clock.schedule_interval(
                c.update,
                1.0 / self.settings['framerate']
            )

        Epiphany_core_grid = GridLayout(cols=4, rows=4, spacing=20, padding=10)
        

        for row in orientation[1:]:
            for i in row:
                c = Core(i, self)
                self.cores[i] = c
                Epiphany_core_grid.add_widget(c)
                Clock.schedule_interval(
                    c.update,
                    1.0 / self.settings['framerate']
                )

        core_grid = GridLayout(rows=2, cols=2, spacing_y=20)

        core_grid.add_widget(BoxLayout(size_hint_x=0.1, size_hint_y=0.25))
        core_grid.add_widget(ARM_core_grid)


        r = BoxLayout(size_hint_x=0.1, spacing=20, orientation='vertical')
        l = Label(
            text='eCore:\n%.3fV' % self.eCore_voltage,
            size_hint_y=0.1,
            halign='center'
        )
        self.voltage_label = l 
        r.add_widget(l)
        
        vs = ValueSlider(
            min=0.900,
            max=1.200,
            value=self.eCore_voltage,
            values=[float(i / 1000.0) for i in range(900, 1225, 25)],
            orientation='vertical',
            size_hint_y=0.8,
            halign='center'
        )
        vs.val = self.eCore_voltage
        vs.bind(on_change=self.voltage_changed)
        vs.bind(on_release=self.voltage_set)
        self.voltage_slider = vs
        r.add_widget(vs)

        core_grid.add_widget(r)
        core_grid.add_widget(Epiphany_core_grid)

        self.layout.add_widget(core_grid)
        
        # grid_scatter = ScatterLayout(do_rotation=False)
        # grid_scatter.add_widget(core_grid)
        # grid_box = Layout()
        # grid_box.add_widget(grid_scatter)
        # self.layout.add_widget(grid_scatter)
    ...
    def init_chip_info(self):
        ...
        chip_info.add_widget(Label(
            text=("Dual-core ARM A9, 16-core Coprocessor"),
            size_hint=(1, .3)
        ))
    ...
    def init_temperature_graph(self):
        """Initialize the chip temperature graph in the lower right corner."""
        ...
        self.temperature_graph = PerfGraph("Temp", unit=u"\u00b0C", 
                color=[.3, 1, 1], history=100, max_y=70, percent_scale=False)
    ...
    def init_power_graph(self):
        ...
        self.power_graph = PerfGraph("Total", extra_content=["A15","A7"], 
            unit="W", percent_scale=False, history=100)

        graph_container.add_widget(self.power_graph)
        # graph_container.add_widget(self.power_graph15)
        self.rightbar.add_widget(graph_container)
        self.power_graph.add_line("A15", 0.5)
        self.power_graph.add_line("A7", 0.7)
    ...
    def show_frequencies(self, *largs):
        ...
        if not self.frequencies_window:
            ...
            for i in xrange(len(self.settings['frequency_islands'])):
                ...
                l = Label(
                    text='Power domain %d:\nFrequency: %dMHz' % (i, 1400),
                    size_hint_x=None,
                    width=150
                )
                self.frequency_labels += [l] 
                r.add_widget(self.frequency_labels[i])
                
                vs = ValueSlider(
                    min=self.settings['frequency_table'][-1],
                    max=self.settings['frequency_table'][0],
                    value=1400,
                    values=self.settings['frequency_table'],
                    data=i
                )
                vs.val = 1400
                ...
                r.add_widget(self.frequency_sliders[i])
                b.add_widget(r)
    ...
    def frequency_changed(self, ins):
        ...
        if ins.data != None:
            ...
            l = self.frequency_labels[len(self.settings['frequency_islands'])]
            l.text = 'All power domains:\nFrequency: %dMHz' % (ins.val)
            for i in xrange(len(self.settings['frequency_islands'])):
                s = self.frequency_sliders[i]
                s.val = ins.val

    def voltage_changed(self, ins):
        Logger.debug("ManyMan: voltage slider changed")
        l = self.voltage_label
        l.text = 'eCore:\n%.3fV' % ins.val
    ...
    def frequency_set(self, ins):
        ...
        if ins.data != None:
            self.comm.set_core_frequency(
                ins.val,
                self.settings['frequency_islands'][ins.data][0]
            )
        else:
            Logger.info("ManyMan: slider %s set" % ins.data)
            self.comm.set_core_frequency(ins.val)

    def voltage_set(self, ins):
        # Logger.debug("ManyMan: voltage slider set")
        Logger.info("ManyMan: voltage slider set")
        self.comm.set_core_voltage(ins.val, 2)
    ...
    def get_cpu_temperature(self):
        """Getter for the current CPU temperature. MAY NOT BE CALLED."""
        raise Exception("Can not access the current CPU temperature.")

    def set_cpu_temperature(self, value):
        """Setter for the current CPU temperature. Updates the temperature graph."""
        self.temperature_graph.update(value)
    ...
    def set_cpu_power(self, value):
        ...
        # self.power_graph.update_line("A15", value["A15"])
        # self.power_graph.update_line("A7", value["A7"])
        # self.power_graph.update(value["A15"] + value["A7"])
        # Logger.debug("PowerGraph: %s" % value)

    # Define getters and setters.
    cpu_load = property(get_cpu_load, set_cpu_load)
    cpu_power = property(get_cpu_power, set_cpu_power)
    cpu_temperature = property(get_cpu_temperature, set_cpu_temperature)
