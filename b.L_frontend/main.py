...
class ManyMan(App):
    ...
    def init_core_grid(self):
        ...
        big_core_grid = GridLayout(cols=2, rows=2, spacing=20, padding=10, 
                                    size_hint_y=0.65)
        for i in orientation[0]:
            c = Core(i, self)
            self.cores[i] = c
            big_core_grid.add_widget(c)
            Clock.schedule_interval(
                c.update,
                1.0 / self.settings['framerate']
            )
        LITTLE_core_grid = GridLayout(cols=4, rows=1, spacing=20, padding=10, 
                                        size_hint_y=0.25)
        for i in orientation[1]:
            c = Core(i, self)
            self.cores[i] = c
            LITTLE_core_grid.add_widget(c)
            Clock.schedule_interval(
                c.update,
                1.0 / self.settings['framerate']
            )
        ...
        r = BoxLayout(size_hint_x=0.1, size_hint_y=0.65, spacing=10, 
                        orientation='vertical')
        l = Label(
            text='A15:\n%dMHz' % 2000,
            size_hint_y=0.1,
        ...
        vs = ValueSlider(
            min=self.settings['frequency_table_A15'][-1],
            max=self.settings['frequency_table_A15'][0],
            value=self.settings['frequency_table_A15'][0],
            values=self.settings['frequency_table_A15'],
            data=0,
            orientation='vertical',
            size_hint_y=0.8,
            halign='center'
        )
        vs.val = self.settings['frequency_table_A15'][0]
        ...
        core_grid.add_widget(big_core_grid)


        r = BoxLayout(size_hint_x=0.1, size_hint_y=0.25, spacing=20, 
                        orientation='vertical')
        l = Label(
            text='A7:\n%dMHz' % 1400,
            size_hint_y=0.1,
        ...
        vs = ValueSlider(
            min=self.settings['frequency_table_A7'][-1],
            max=self.settings['frequency_table_A7'][0],
            value=self.settings['frequency_table_A7'][0],
            values=self.settings['frequency_table_A7'],
            data=1,
            orientation='vertical',
            size_hint_y=0.8,
            halign='center'
        )
        vs.val = self.settings['frequency_table_A7'][0]
        vs.bind(on_change=self.frequency_changed)
        vs.bind(on_release=self.frequency_set)
        self.frequency_sliders += [vs]
        r.add_widget(vs)
        ...
        core_grid.add_widget(LITTLE_core_grid)
