...
default_settings = {
    ...
    'epiphany_status_dir': '/home/linaro/',
    'erm_path': '/home/linaro/epiphany-examples/apps/erm/run.sh',
    'eVolt_command': 'sudo /home/linaro/Documents/parallella-utils-master/power_management/evolt',
    'voltage_timeout': 3,
    'status_frequency': 1,
    'frequency_timeout': 3,
    'chip_name': 'Parallella',
    'chip_cores': 18,
    'chip_orientation': [
        [0, 1],
        [2, 3, 4, 5],
...
class Server(SocketServer.TCPServer):
    ...
    def init_voltage_handler(self):
        """Initialize the voltage handler."""
        self.voltage_handler = VoltageHandler(self, self.settings)
        self.voltage_thread = Thread(
            target=self.voltage_handler.wait_for_assignment
        )
        self.voltage_thread.deamon = True
        self.logger.info("Initialized the VoltageHandler")
    ...
    def serve_forever(self, max_lines):
        ...
        try:
            ...
            self.voltage_handler.set_eCore_voltage(1.00)
            sleep(2) # Give the handler time to update
            self.voltage_handler.running = False
            self.voltage_thread.join()
            self.logger.info("Stopped the VoltageHandler")
            self.logger.info("Stopped")
...
class VoltageHandler:
    ...
    def __init__(self, server, settings):
        self.server = server
        self.settings = settings
        self.logger = logging.getLogger('VoltageHandler')
        self.running = True
        self.voltages = [1.00] * 2
        self.last_change = time()
        self.changed = False
        self.changed_island = None
    ...
    def update_voltage(self):
        self.logger.info("Updating voltage")

        if self.changed_island != None:
            cmd = '%s -e %.3f' % (
                self.settings['eVolt_command'],
                self.voltages[self.changed_island]
            )
            ...
            if len(err) > 0:
                self.logger.warning(
                    "Error when setting voltage: %s" % err
                )
    ...
    def set_eCore_voltage(self, v, core=None):
        if time() - self.last_change < self.settings['voltage_timeout']:
            self.logger.warning("Too little time between voltage changes.")
            return
        ...
        if core != None:
            ...
            for c in self.settings['voltage_islands'][i]:
                self.server.chip.cores[c].voltage = v
        else:
    ...
    def get_eCore_voltage(self):
        """Retrieve the Epiphany voltage setting from EVolt."""
        p = sp.Popen(
            self.settings['eVolt_command'],
            shell=True,
            stdout=sp.PIPE,
        ...
        if len(err) > 0:
            self.logger.warning(
                "Error when getting voltage: %s" % err
            )

        for line in out.split('\n'):
            if 'Epiphany Core:' in line:
...
class App:
    ...
    def init_erm(self):
        """Start up the eCore resource monitor."""
        if not self.settings['dummy_mode']:

            self.erm = sp.Popen(self.settings['erm_path'], 
                shell=True,
                stdout=sp.PIPE,
                stderr=sp.PIPE
            )
    ...
    def init_chip(self):
        ...
        self.chip = Chip(
            ...
            status_dir=self.settings['epiphany_status_dir'],
