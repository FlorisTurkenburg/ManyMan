...
class Core(Thread):
    ...
    def setup(self):
        """Execute mpstat on the given core."""
    ...
    def run(self):
        ...
        while 1:
            ...
            for task in self.tasks.values():
                try:
                    if task.status != TaskStatus.KILLING and task.p_top != None:
    ...
    def kill_task(self, t):
        ...
        if t.kill():
            output = t.output
            self.logger.warning("Going to join task!!")
            t.join()
            self.logger.warning("Task Joined!!")
            return output
